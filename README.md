...
## 站点演示
[www.turbo.kim](www.turbo.kim)

## 模块分层
### 后端模块
```shell
MyBlog
├── turbo-blog-auth   # 鉴权模块：shiro
│   ├── pom.xml
│   └── src
├── turbo-blog-core   # 核心模块：配置文件，Entity类，mapper类，工具类，异常过滤等
│   ├── pom.xml
│   └── src
├── turbo-blog-manage # 后台管理界面Service
│   ├── pom.xml
│   └── src
├── turbo-blog-portal # 前端界面Service
│   ├── pom.xml
│   └── src
├── turbo-blog-search # 搜索模块：elasticSearch
│   ├── pom.xml
└── └── src
```
### 后台依赖关系

turbo-blog-core -> turbo-blog-auth -> turbo-blog-manage -> turbo-blog-portal -> turbo-blog-search
